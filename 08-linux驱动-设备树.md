- [1. Linux设备树时钟配置解析：以RK3568 CRU为例](#1-linux设备树时钟配置解析以rk3568-cru为例)
  - [1.1. 时钟生产者与消费者模型](#11-时钟生产者与消费者模型)
  - [1.2. CRU节点的关键属性](#12-cru节点的关键属性)
    - [1.2.1. `#clock-cells`](#121-clock-cells)
    - [1.2.2. `assigned-clocks`、`assigned-clock-parents`、`assigned-clock-rates`](#122-assigned-clocksassigned-clock-parentsassigned-clock-rates)
  - [1.3. 为什么CRU节点没有逐一列出所有时钟，却仍能被引用？](#13-为什么cru节点没有逐一列出所有时钟却仍能被引用)
  - [1.4. 完整引用示例：GMAC时钟配置](#14-完整引用示例gmac时钟配置)
  - [1.5. 总结](#15-总结)
...
# 1. Linux设备树时钟配置解析：以RK3568 CRU为例
...
### 1.2.2. `assigned-clocks`、`assigned-clock-parents`、`assigned-clock-rates`
