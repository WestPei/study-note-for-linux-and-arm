- [1. Yocto 核心概念](#1-yocto-核心概念)
  - [1.1. 配方 (.bb / .bbappend)](#11-配方-bb--bbappend)
  - [1.2. 配置文件 (.conf)](#12-配置文件-conf)
    - [1.2.1. 配方与配置文件的关系](#121-配方与配置文件的关系)
  - [1.3. 层 (Layer)](#13-层-layer)
  - [1.4. 构建目录 (build/)](#14-构建目录-build)
- [2. 构建流程与任务链](#2-构建流程与任务链)
  - [2.1. 完整构建流程](#21-完整构建流程)
  - [2.2. 默认任务链详解](#22-默认任务链详解)
    - [2.2.1. do\_fetch — 获取源码/补丁](#221-do_fetch--获取源码补丁)
    - [2.2.2. do\_unpack — 解压/检出源码](#222-do_unpack--解压检出源码)
    - [2.2.3. do\_patch — 打补丁](#223-do_patch--打补丁)
    - [2.2.4. do\_configure — 配置编译参数](#224-do_configure--配置编译参数)
    - [2.2.5. do\_compile — 编译](#225-do_compile--编译)
    - [2.2.6. do\_install — 安装到伪根文件系统](#226-do_install--安装到伪根文件系统)
    - [2.2.7. do\_package / do\_rootfs — 打包与生成镜像](#227-do_package--do_rootfs--打包与生成镜像)
    - [2.2.8. do\_install 之后为什么还需要 do\_package？](#228-do_install-之后为什么还需要-do_package)
    - [2.2.9. do\_rootfs 干了什么？](#229-do_rootfs-干了什么)
- [3. 关键目录与变量](#3-关键目录与变量)
  - [3.1. 四个核心目录变量](#31-四个核心目录变量)
  - [3.2. 变量与赋值操作符](#32-变量与赋值操作符)
  - [3.3. 任务标记 (Varflags)](#33-任务标记-varflags)
  - [3.4. 任务修改](#34-任务修改)
- [4. virtual/kernel 等虚拟目标机制](#4-virtualkernel-等虚拟目标机制)
  - [4.1. 什么是虚拟目标](#41-什么是虚拟目标)
  - [4.2. Provider 模式](#42-provider-模式)
  - [4.3. 解析流程](#43-解析流程)
    - [4.3.1. 连接机制：PROVIDES 变量](#431-连接机制provides-变量)
    - [4.3.2. 选择机制：PREFERRED\_PROVIDER 与版本规则](#432-选择机制preferred_provider-与版本规则)
  - [4.4. 确定 virtual/\* 指向的配方](#44-确定-virtual-指向的配方)
  - [4.5. 配方如何声明自己是 virtual/kernel](#45-配方如何声明自己是-virtualkernel)
  - [4.6. 查找具体配方文件位置](#46-查找具体配方文件位置)
- [5. 工程结构规范](#5-工程结构规范)
  - [5.1. 标准 Yocto 工程结构](#51-标准-yocto-工程结构)
  - [5.2. 自定义层的组织结构](#52-自定义层的组织结构)
  - [5.3. meta-bsp vs meta-sdk vs local.conf](#53-meta-bsp-vs-meta-sdk-vs-localconf)
    - [5.3.1. 我应该修改哪个配置文件？](#531-我应该修改哪个配置文件)
  - [5.4. 层配置文件 (layer.conf) 示例](#54-层配置文件-layerconf-示例)
  - [5.5. 整改已有工程](#55-整改已有工程)
- [6. 初始化脚本与工程迁移](#6-初始化脚本与工程迁移)
  - [6.1. 官方初始化脚本](#61-官方初始化脚本)
  - [6.2. 自定义构建目录初始化脚本](#62-自定义构建目录初始化脚本)
  - [6.3. setup-env 项目初始化脚本](#63-setup-env-项目初始化脚本)
  - [6.4. 工程迁移：打包-传输-解压](#64-工程迁移打包-传输-解压)
  - [6.5. 离线构建配置](#65-离线构建配置)
- [7. SVN 集成专题](#7-svn-集成专题)
  - [7.1. 在 distro 中配置 SVN 镜像重定向](#71-在-distro-中配置-svn-镜像重定向)
  - [7.2. 配方中配置 SVN 下载](#72-配方中配置-svn-下载)
    - [7.2.1. 源码目录匹配问题](#721-源码目录匹配问题)
    - [7.2.2. 使用 .inc 文件统一管理公共变量](#722-使用-inc-文件统一管理公共变量)
    - [7.2.3. cfg 配置碎片文件路径问题](#723-cfg-配置碎片文件路径问题)
    - [7.2.4. 跳过 do\_patch](#724-跳过-do_patch)
  - [7.3. local-git 类的干扰](#73-local-git-类的干扰)
  - [7.4. SVN 证书问题](#74-svn-证书问题)
    - [7.4.1. SVN 密码问题](#741-svn-密码问题)
  - [7.5. bitbake 使用本地 SVN](#75-bitbake-使用本地-svn)
  - [7.6. SVN 配置层次总结](#76-svn-配置层次总结)
    - [7.6.1. SVN 最终解决](#761-svn-最终解决)
  - [7.7. 清除缓存注意事项](#77-清除缓存注意事项)
- [8. \*-native 概念](#8--native-概念)
- [9. Applications 用户态工具编译](#9-applications-用户态工具编译)
  - [9.1. 编译报错：递归 Make + 并行竞态](#91-编译报错递归-make--并行竞态)
- [10. VScode 远程连接开发](#10-vscode-远程连接开发)
  - [10.1. SSH 配置](#101-ssh-配置)
  - [10.2. SSH 密钥免密登录](#102-ssh-密钥免密登录)
- [11. 附录：SVN 基本操作](#11-附录svn-基本操作)
...
### 2.2.7. do_package / do_rootfs — 打包与生成镜像
...
### 2.2.8. do_install 之后为什么还需要 do_package？
...
### 4.3.2. 选择机制：PREFERRED_PROVIDER 与版本规则
